...
> **GUI 工作流程**: 抓取 → Markdown 文件（按周分组，图片 base64 内嵌）。  
> 支持 GUI 模式（`python gui.py`）和命令行模式（`python weibo.py`），输出 Markdown 文件。
...
连续爬取**一个**或**多个**新浪微博用户（如[Dear-迪丽热巴](https://weibo.cn/u/1669879400)、[郭碧婷](https://weibo.cn/u/1729370543)）的数据，并将结果写入 **Markdown 文件**。支持以下功能：

* 写入**Markdown文件**（按周分组）
* 下载用户**原创**微博中的原始**图片**（可选，base64 嵌入 MD）
* 下载用户**转发**微博中的原始**图片**（可选，base64 嵌入 MD）

程序支持**增量爬取**（append 模式），基于 `since_date` 过滤已爬取的微博。

## GUI 界面说明

运行 `python gui.py` 启动图形界面，提供以下功能：
...
| **暂停/继续** | 支持断点续传 |
| **停止** | 安全停止并保存已获取数据 |
| **测试(3条)** | 仅下载选中用户 3 条微博，快速验证 Cookie 是否有效（忽略反爬延迟） |
| **📝 批量保存为 MD** | 粘贴微博链接逐条抓取，保存为 Markdown 文件 |
| **🔍 关键词过滤** | 支持多关键词（逗号/顿号/空格/分号分隔），正文或话题标签含任一关键词即保存（OR 关系，不区分大小写） |
| **📂 关键词分组** | 保存为命名分组（如"蔚来相关"），一键应用/从文件导入/最近使用记录 |
| **👥 用户分组** | 保存为命名分组（如"蔚来黑子"），一键应用/替换/合并/从文件导入/最近使用恢复 |
| **📋 已抓取用户列表** | 自动生成统计报告，含用户基本信息、粉丝数、抓取微博数 |
| **✂ 分割大MD** | 扫描超过 10MB 的 Markdown 文件，按二级标题自动分割为小文件 |

### 工作流程

```
抓取微博 → Markdown 文件（按周分组，图片 base64 内嵌）
        ↓
  Markdown 阅读器 / VS Code 查看
```

### 输出模式

- **Markdown 按周分组**：微博按 ISO 周（周一~周日）组织为独立 `.md` 文件

### 追加模式

- 基于 `since_date` 过滤，已抓取的旧微博不再重复获取
- 支持多账号独立管理
...
* 结果文件：保存在 `weibo_data/用户昵称/` 目录下，按配置的分组方式（天/周/月）生成 `.md` 文件
* 微博图片：下载后以 base64 内嵌到 Markdown 文件中，不保留独立文件
...
> **GUI 关键词过滤**（`keyword_filter`）是更灵活的本地方案：抓取全部微博后在本地按关键词过滤，不依赖微博搜索接口。多关键词用逗号、顿号、空格、分号等分隔（如 `AI, 人工智能、NLP`），正文或话题标签匹配**任一**即保存（OR 关系，不区分大小写）。勾选 `🔍 关键词` 复选框并填入关键词即可启用，无需修改 `config.json`。
> 
> **📂 关键词分组管理**：可将常用关键词保存为命名分组（如"蔚来相关"含"蔚来, NIO, 换电, 电池租用"），之后一键应用。支持从 .txt 文件批量导入关键词、自动记录最近使用。
> 
> **👥 用户分组管理**：可将当前用户列表保存为命名分组（如"蔚来黑子"含多位博主），之后一键应用（替换或合并）。支持从 .txt 文件批量导入用户（每行 `user_id nickname`）、自动记录最近使用的用户组合。适合管理几百上千个目标用户的场合。
