...
| **🔍 关键词过滤** | 支持多关键词（逗号分隔），正文或话题标签含任一关键词即保存（OR 关系） |
...
> **GUI 关键词过滤**（`keyword_filter`）是更灵活的本地方案：抓取全部微博后在本地按关键词过滤，不依赖微博搜索接口。多关键词用逗号分隔（如 `AI, 人工智能, NLP`），正文或话题标签匹配**任一**即保存（OR 关系）。勾选 `🔍 关键词` 复选框并填入关键词即可启用，无需修改 `config.json`。
